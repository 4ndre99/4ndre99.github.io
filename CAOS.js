$(document).ready(function () {
    $("form").submit(function () {
        ...
        $("#maxValue").val(max);
        if ($('#inputNome').val().length < 3 || $('#inputNome').val().length > 100) {
            $("#myModal ul").append('<li>O nome deve ter entre 3 e 100 letras.</li>');
            retVal = false;
        }
        if ($('#inputEndereco').val().length < 20 || $('#inputEndereco').val().length > 200) {
       ...
       if ($('input[type="checkbox"]:checked').length < 1){
            $("#myModal ul").append('<li>Deverá escolher, pelo menos, 1 opção.</li>');
           retVal = false;
       }
